...
import { registrarInscripcion, listarInscripcionesPorTorneo } from "../api";
...
export const useInscripcion = (torneoId, categoriaRequerida) => {
    ...
    const actualizarCompetidores = async () => {
        try {
            const lista = await listarInscripcionesPorTorneo(torneoId);
    ...
    const inscribir = async (robot, usuario) => {
        ...
        try {
            ...
            const response = await registrarInscripcion(data); // Backend call
    ...
    const retirar = async (usuarioId) => {
        try {
            // Si tenemos ID de inscripción, usamos ese. Si no, quizá el usuario ID.
            // Lo ideal es usar el ID de la inscripción (inscripcionId)
            const idParaBorrar = inscripcionId || usuarioId;

            // TODO: No hay endpoint de DELETE para inscripciones en el backend
            // await retirarInscripcion(idParaBorrar);
            console.warn('Retirar inscripción no está implementado en el backend');
